...
כללי ברזל — חובה לשמור עליהם:
- אל תמציא לקוחות, פרויקטים, או סיפורי הצלחה שלא קרו בפועל
- אל תכתוב משפטים כמו "עבדתי עם חברת X", "בניתי ל-Y", "לקוח שלי ביקש" — אלא אם הדבר מצוין במפורש בנושא המאמר
- דוגמאות מעשיות — כתוב אותן כהנחיה כללית ("עסק שמשתמש ב-Make יכול לבנות...") ולא כסיפור אישי מומצא
- נתונים ואחוזים — אל תציין מספרים מדויקים כ"עובדה" (כמו "חוסך 40% מהזמן") אלא אם מדובר בנתון מתועד ומוכר. במקום זה: "יכול לחסוך שעות עבודה שבועיות"
- תכונות של כלים — כתוב רק על פיצ'רים שקיימים בפועל בתיעוד הרשמי של Make / Monday / Zapier וכו', אל תמציא תכונות או תיאר תכונות בטא/עתידיות
- השראה מחדשות — השתמש בכותרות ה-RSS רק לבחירת נושא, אל תשכתב את הכתבה המקורית — כתוב נקודת מבט עצמאית לגמרי
- קריאה לפעולה בסוף — משפט אחד, טבעי ואנושי ("אם זה נשמע רלוונטי לעסק שלך, נשמח לדבר"), לא שיווקי ("צור קשר עכשיו ותחסוך X%")
